...
public class DataImporter {
    public static void main(String[] args) {
        String filePath = "auto-mpg.txt"; 
        String url = "jdbc:mysql://localhost:3306/Auto";
        String user = "testuser";
        String pass = "Password";

        String insertSQL = "INSERT INTO mpg_data (mpg, cylinders, displacement, horsepower, weight, acceleration, model_year, origin, car_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conn = DriverManager.getConnection(url, user, pass);
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (m.find()) {
                    String numberPart = m.group(1).trim();
                    String carName = m.group(2).trim();

                    String[] parts = numberPart.split("\\s+");

                    if (parts.length >= 8) {
                        try {
                            ps.setFloat(1, (float) Double.parseDouble(parts[0])); // mpg
                            ps.setInt(2, (int) Double.parseDouble(parts[1]));   // cylinders
                            ps.setFloat(3, (float) Double.parseDouble(parts[2])); // displacement
                            
                            String hp = parts[3];
                            ps.setFloat(4, hp.equals("?") ? 0.0f : (float) Double.parseDouble(hp));
                            
                            ps.setFloat(5, (float) Double.parseDouble(parts[4])); // weight
                            ps.setFloat(6, (float) Double.parseDouble(parts[5])); // acceleration
                            ps.setInt(7, (int) Double.parseDouble(parts[6]));    // model_year
                            ps.setInt(8, (int) Double.parseDouble(parts[7]));    // origin
                            ps.setString(9, carName);

                            ps.executeUpdate();
                            count++;
                        } catch (Exception e) {
                            System.err.println("Skipping malformed data in row: " + line);
                        }
                    }
                } else {
                    System.err.println("Could not find quoted car name in: " + line);
                }
            }
...
public class AutoDataGUI extends JFrame {
    private JTextField queryField;
    private JSlider mpgSlider, hpSlider;
    private JButton searchButton, sliderButton;
    
    private String url = "jdbc:mysql://localhost:3306/Auto";
    private String user = "testuser";
    private String pass = "Password";

    public AutoDataGUI() {
        setTitle("Auto Database Pro");
        setSize(500, 350);
        ...
        textPanel.setBorder(BorderFactory.createTitledBorder("Search by Name"));
        queryField = new JTextField(15);
        searchButton = new JButton("Search Name");
        textPanel.add(new JLabel("Name/ALL:"));
        textPanel.add(queryField);
        textPanel.add(searchButton);

        //Slider Search
        JPanel sliderPanel = new JPanel(new GridLayout(3, 1));
        sliderPanel.setBorder(BorderFactory.createTitledBorder("Filter by Performance"));

        //MPG Slider
        mpgSlider = new JSlider(9, 50, 20);
        mpgSlider.setMajorTickSpacing(10);
        mpgSlider.setPaintTicks(true);
        mpgSlider.setPaintLabels(true);
        
        //HP Slider
        hpSlider = new JSlider(40, 250, 100);
        hpSlider.setMajorTickSpacing(50);
        hpSlider.setPaintTicks(true);
        hpSlider.setPaintLabels(true);

        sliderButton = new JButton("Filter by Sliders");

        sliderPanel.add(new JLabel("Minimum MPG:"));
        sliderPanel.add(mpgSlider);
        sliderPanel.add(new JLabel("Minimum Horsepower:"));
        ...
        searchButton.addActionListener(e -> {
            ...
            if (input.equalsIgnoreCase("ALL")) {
                displayResults("SELECT * FROM mpg_data", null);
            } else {
                displayResults("SELECT * FROM mpg_data WHERE car_name LIKE ?", "%" + input + "%");
            }
        });
        ...
        sliderButton.addActionListener(e -> {
            ...
            //SQL query using two WHERE conditions
            String sql = "SELECT * FROM mpg_data WHERE mpg >= ? AND horsepower >= ?";
            displayResultsWithSliders(sql, minMpg, minHp);
        });
    }

    private void displayResults(String sql, String param) {
        runQuery(sql, param, -1, -1);
    }

    private void displayResultsWithSliders(String sql, int mpg, int hp) {
        runQuery(sql, null, mpg, hp);
    }

    private void runQuery(String sql, String textParam, int mpgParam, int hpParam) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new AutoDataGUI().setVisible(true));
    }
}
